...
//-----------------------------------------------------------------------------------------------------------------------------------------
float CSG_ConeIntersect( vec3 p0, float r0, vec3 p1, float r1, vec3 rayOrigin, vec3 rayDirection, out vec3 n1, out vec3 n2, out float far )
//-----------------------------------------------------------------------------------------------------------------------------------------
{
	r0 += 0.1;
	float result = INFINITY;
...
//---------------------------------------------------------------------------------------
{
	...
	A_near = CSG_BoxIntersect( vec3(-260, 0, 195), vec3(-230, 40, 225), rayOrigin, rayDirection, A_n1, A_n2, A_far );
	B_near = CSG_ConeIntersect( vec3(-235, 25, 240), 5.0, vec3(-240, 5, 170), 0.1, rayOrigin, rayDirection, B_n1, B_n2, B_far );
	...
	A_near = CSG_BoxIntersect( vec3(-260, 0, 95), vec3(-230, 40, 125), rayOrigin, rayDirection, A_n1, A_n2, A_far );
	B_near = CSG_ConeIntersect( vec3(-235, 25, 140), 12.0, vec3(-240, 5, 70), 0.1, rayOrigin, rayDirection, B_n1, B_n2, B_far );
	d = operation_HollowA_Plus_HollowB( A_near, A_far, B_near, B_far, A_n1, A_n2, B_n1, B_n2, n );
	if (d < t)
	{
		t = d;
	...
	A_near = CSG_BoxIntersect( vec3(-260, 0, -15), vec3(-230, 40, 15), rayOrigin, rayDirection, A_n1, A_n2, A_far );
	B_near = CSG_ConeIntersect( vec3(-235, 25, 30), 20.0, vec3(-240, 5, -40), 0.1, rayOrigin, rayDirection, B_n1, B_n2, B_far );
	d = operation_HollowA_Plus_HollowB( A_near, A_far, B_near, B_far, A_n1, A_n2, B_n1, B_n2, n );
	if (d < t)
	{
		t = d;
